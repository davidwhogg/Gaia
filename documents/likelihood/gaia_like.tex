...
and the function is symmetric in parallax space---not distance space---because
...
\begin{eqnarray}
p(y\given X_n) &=& A_n\,N(X_n\given\hat{X}_n,\hat{C}_{X,n})
\label{eq:like}
\quad ,
\end{eqnarray}
where now
...
$N(\xi\given\mu,V)$ is now the $D$-dimensional Gaussian function for $\xi$ given mean $D$-vector$\mu$ and $D\times D$ covariance matrix $V$,
$\hat{X}_n$ is the $D$-vector of values given for the $D$ astrometric quantities for star $n$ in the \Gaia\ Catalog,
and $\hat{C}_{X,n}$ is the value given for the $D\times D$ covariance matrix for that
$D$-vector.
...
HOGG: There are nuisance parameters. We are going to assume that the Gaia team either
